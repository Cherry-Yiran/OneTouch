...
- 一键正常退出其他 Dock 应用，并保留当前工作应用、OneTouch 与 Finder
- 推出外置物理磁盘，并支持磁盘保护名单
- 登录时启动
- 为任意控制项录制全局快捷键
...
- “关闭其他应用”只发送 macOS 标准退出请求；存在未保存内容时，由对应应用正常询问，不会强制结束进程
- 在“偏好设置 → 自定义”中选择和排序控制项
- 在“偏好设置 → 快捷键”中录制全局快捷键

OneTouch 会在首次启动时先引导完成核心功能所需的辅助功能授权，授权成功后自动进入主面板，避免后续操作反复被权限流程打断。自动化、蓝牙或专注状态等专项权限仍只在用户启用对应能力前请求。不支持的硬件能力和未安装的应用会直接显示为不可用，不会跳转到无关设置页面。
